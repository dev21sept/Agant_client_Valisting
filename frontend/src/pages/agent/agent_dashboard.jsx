    ...
    const fetchAgentData = async () => {
        try {
            ...
            const [perf, clients] = await Promise.all([
                getAgentPerformance(user.id),
                getClients()
            ]);
            setAgentPerf(perf);
            const client = clients.find(c => c._id === user.clientId);
            setMyClient(client);
        } catch (error) {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                        Welcome Back, {user.name} <Sparkles className="text-amber-400 w-8 h-8" />
                    </h1>
                    <p className="text-gray-500 mt-1 font-medium italic text-sm">Here is your performance snapshot for today.</p>
                </div>
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                    <div className="relative z-10">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Active Client</span>
                        </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Default Rule</p>
                                        <p className="text-xs font-bold">{myClient.defaultRules?.price_markup || 0}% Markup Applied</p>
                                    </div>
                                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Condition Notes</p>
                                        <p className="text-xs font-bold truncate">{myClient.defaultRules?.condition_note || 'Standard'}</p>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            <div className="text-center py-10 bg-white/5 rounded-3xl border border-dashed border-white/20">
                                <p className="text-slate-400 font-bold italic">No client assigned yet. Contact Admin.</p>
                            </div>
                        )}
                    </div>
